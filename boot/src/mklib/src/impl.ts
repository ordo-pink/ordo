...
import { TLicenseType, create_progress, create_repository_file as echo, get_license, get_spdx_record } from "@ordo-pink/binutil"
...
const init_progress: Unary<string, void> = name => progress.start(`Initializing new library "${name}"`)

const create_files: Ternary<string, string, TLicenseType, Thunk<Oath<void, Error>>> = (path, name, license) => () =>
	Oath.Merge([
...
const createFilesIfNotExists0: Binary<string, TLicenseType, Unary<string, Oath<void, string | Error>>> =
	(name, license) => path =>
		dir_exists0(path)

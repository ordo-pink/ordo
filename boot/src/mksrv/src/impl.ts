...
import {
	create_repository_file as create,
	create_progress,
	get_license,
	get_spdx_record,
...
export const mksrv = (name: string, license: TLicenseType) =>
	Oath.Resolve(isReservedJavaScriptKeyword(name) ? `${name}-srv` : name)
		...
		.pipe(
			ops0.chain(name =>
				Oath.Resolve(get_absolute_path(`./srv/${name}`)).pipe(
					ops0.chain(create_file_if_not_exists(name, license)),
				),
			),
		)
		.invoke(
...
const create_file_if_not_exists: Binary<
	string,
	TLicenseType,

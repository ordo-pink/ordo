...
import { check_files_exist, dirents_to_dirs, get_dirent_names, get_existing_paths, run_async_command } from "@ordo-pink/binutil"
...
export const run = () =>
	readdir0("./srv", { withFileTypes: true })
		...
		.pipe(
			ops0.chain(paths =>
				Oath.Merge(paths.map(path => run_async_command(`opt/bun run ${path}`, { stdout: "pipe", stderr: "pipe" }))).pipe(
					ops0.map(noop),
				),
			),
		)

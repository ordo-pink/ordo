...
const check_email_is_not_taken_if_present = (body: Record<string, any>, i: I) =>
	body.email
		? oath
				...
				.pipe(
					oath.ops.chain(id => oath.if_else(!id || id === i.params.user_id, { f: () => exists_by_email_rrr(body.email, i) })),
				)
		: oath.of(void 0)
...
const check_handle_is_not_taken_if_present = (body: Record<string, any>, i: I) =>
	body.handle
		? oath
				...
				.pipe(oath.ops.chain(id => oath.if_else(!id || id === i.params.user_id, { f: () => exists_by_handle(body.handle, i) })))
		: oath.of(void 0)

const check_installed_functions_is_valid_if_present = (body: Record<string, any>, i: I) =>
	body.installed_functions
		? oath.if_else(is_installed_functions(body.installed_functions), {
...
const check_first_name_is_valid_if_present = (body: Record<string, any>, i: I) =>
	body.first_name
		? oath.if_else(is_first_name(body.first_name), { f: () => invalid_first_name_rrr(body.first_name, i) })
		: oath.of(void 0)

const check_last_name_is_valid_if_present = (body: Record<string, any>, i: I) =>
	body.last_name
		? oath.if_else(is_last_name(body.last_name), { f: () => invalid_last_name_rrr(body.last_name, i) })
		: oath.of(void 0)

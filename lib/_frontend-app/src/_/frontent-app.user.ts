...
export const init_user = call_once(() => {
	...
	oath
		...
		.pipe(oath.ops.and(res => oath.if_else(res.success, { t: () => res.payload as Ordo.User.Current.DTO })))
	...
	ordo_app_state.zags.cheat("user", user => {
		if (user) {
			...
			commands.emit("cmd.application.command_palette.add", {
				...
				render_icon: bs_box_arrow_in_right,
			})
		}
	})
		...
		Dialog({
			action: () =>
				oath
					...
					.pipe(oath.ops.and(res => res.json()))
					.pipe(oath.ops.and(res => oath.if_else(res.success)))
			...
			Dialog({
				action: () =>
					oath
						...
						.pipe(oath.ops.and(res => res.json()))
						.pipe(oath.ops.and(res => oath.if_else(res.success, { t: () => res.payload })))

...
import { InternalUser, UserRepository } from "@ordo-pink/backend-user-service"
import { readFile0, writeFileRecursive0 } from "@ordo-pink/fs"
import { Oath } from "@ordo-pink/oath"

const getUsers0 = (path: string): Oath<InternalUser[], Error> =>
	readFile0(path, "utf-8")
		.fix(() => writeFileRecursive0(path, "[]", "utf-8").map(() => "[]"))
		.chain(file => Oath.try(() => JSON.parse(file as string)))

const saveUsers0 = (path: string, data: InternalUser[]) =>

...
import { TEmailService } from "@ordo-pink/backend-email-service"

// --- Public ---

type Body = { email?: string; password?: string }
type Params = { userService: UserService; tokenService: TTokenService; emailService: TEmailService }
		...
		parseBody0<Body>(ctx)
			...
			.chain(user =>
				Oath.of({ sub: user.id })
					...
					.tap(() =>
						emailService.onSignInFromNewIP({
							email: user.email,
							name: user.firstName ?? "",
							ip: ctx.request.ip,
						}),

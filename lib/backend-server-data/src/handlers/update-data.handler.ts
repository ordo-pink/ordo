...
export const handleUpdateData: Unary<{ dataService: TDataCommands<Readable>; idHost: string }, Middleware> =
	({ dataService, idHost }) =>
	ctx =>
		authenticate0(ctx, idHost)
			...
			.chain(({ sub }) =>
				parse_body0<PlainData>(ctx).chain(data =>
					Oath.of({ fsid: ctx.params.fsid as FSID, createdBy: ctx.params.userId as SUB }).chain(({ fsid, createdBy }) =>
						dataService.update({ fsid, createdBy, updatedBy: sub, data }).rejectedMap(HttpError.NotFound),
					),
				),
			)

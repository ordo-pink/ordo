...
export const handleUploadContent: Unary<{ dataService: TDataCommands<Readable>; idHost: string }, Middleware> =
	({ dataService, idHost }) =>
	ctx =>
		authenticate0(ctx, idHost)
			...
			.chain(
				Oath.ifElse(payload => Number(ctx.req.headers["content-length"]) / 1024 / 1024 <= payload.fms, {
					onFalse: () => HttpError.PayloadTooLarge("File too large"),
				}),
			)
			.chain(({ sub, lim }) =>
				Oath.of({ name: ctx.params.name as string, createdBy: ctx.params.userId as SUB }).chain(({ name, createdBy }) =>

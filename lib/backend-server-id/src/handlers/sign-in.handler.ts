...
type TFn = (ctx: Context, params: TParams) => Oath<Routes.ID.SignIn.Response, TRrr<"ENOENT" | "EIO" | "EINVAL">>
type TCtx = { email: string; handle?: string; password: string } | { email?: string; handle: string; password: string }
...
const validate_ctx0 =
	...
	({ email, handle, password }: TCtx) =>
		Oath.Merge([
			...
			.pipe(Oath.ops.chain(from_option0(() => enoent(`validate_ctx -> ${email ? "email" : "handle"}: ${email ?? handle}`))))

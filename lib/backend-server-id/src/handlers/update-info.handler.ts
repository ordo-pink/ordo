...
import { get_first_name0, get_last_name0 } from "../fns/getters.fn"
...
type TRequest = { token: TAccessJWT; body: Routes.ID.UpdateInfo.RequestBody }
...
const extract_ctx0 =
	...
	({
		...
		token,
	}: TRequest): Oath<TCtx, TRrr<"EINVAL" | "ENOENT" | "EIO">> =>
		Oath.Merge({
			user: user_service
				.get_by_id(token.payload.sub)
				.pipe(
					Oath.ops.chain(
						from_option0(() => Oath.Reject(enoent(`extract_ctx -> sub: ${token.payload.sub}`))),
					),
				),
			first_name: get_first_name0(first_name),
			last_name: get_last_name0(last_name),
		})

const update_user_info0 =
	(user_service: TUserService) =>
	({ first_name, last_name, user }: TCtx): Oath<User.User, TRrr<"EIO" | "ENOENT">> =>
		user_service

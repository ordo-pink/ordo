...
export const sidebar_toggle = maoka.create("button", ({ use }) => {
	...
	use(ordo_client_maoka.jabs.listen("click", () => hunter.shoot("@ordo/main.sidebar.toggle")))
...
export const sidebar = maoka.create("aside", ({ use, node }) => {
	...
	const handle_click = () => is_mobile && hunter.shoot("@ordo/main.sidebar.hide")

	const handle_disable: OrdoClient.Command.GunFor<"@ordo/main.sidebar.disable"> = () => sidebar$.update("enabled", () => false)
	const handle_enable: OrdoClient.Command.GunFor<"@ordo/main.sidebar.enable"> = () => sidebar$.update("enabled", () => true)
	const handle_hide: OrdoClient.Command.GunFor<"@ordo/main.sidebar.hide"> = () => sidebar$.update("visible", () => false)
	const handle_show: OrdoClient.Command.GunFor<"@ordo/main.sidebar.show"> = () => sidebar$.update("visible", () => true)
	const handle_toggle: OrdoClient.Command.GunFor<"@ordo/main.sidebar.toggle"> = () =>
		void (sidebar$.select("enabled") && sidebar$.update("visible", prev => !prev))

	use(ordo_client_maoka.jabs.listen("click", () => handle_click()))
	use(ordo_client_maoka.jabs.handle_command("@ordo/main.sidebar.disable", handle_disable))
	use(ordo_client_maoka.jabs.handle_command("@ordo/main.sidebar.enable", handle_enable))
	use(ordo_client_maoka.jabs.handle_command("@ordo/main.sidebar.hide", handle_hide))
	use(ordo_client_maoka.jabs.handle_command("@ordo/main.sidebar.show", handle_show))
	use(ordo_client_maoka.jabs.handle_command("@ordo/main.sidebar.toggle", handle_toggle))
	...
	return () => {
		...
		if (current_activity && current_activity.render_sidebar) hunter.shoot("@ordo/main.sidebar.enable")
		else hunter.shoot("@ordo/main.sidebar.disable")
		...
		if (!visible || !enabled || !maoka_dom.node_guard(node) || !node.value.parentElement) {
			...
			hunter.shoot("@ordo/main.command_palette.add", {
				...
				value: () => hunter.shoot("@ordo/main.sidebar.toggle"),
				hotkey: "mod+b",

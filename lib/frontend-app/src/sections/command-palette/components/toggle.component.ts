...
export const command_palette_toggle = create.create("div", ({ use, node }) => {
	const { hunter } = use(client_maoka.context.consume)

	const render_icon = (span: HTMLSpanElement) => create.dom.render(span, bs_menu_button_wide_fill(), node.root.create_id)
	...
	const handle_keydown = (event: KeyboardEvent) => {
		if (create.dom.node_guard(node) && event.code === "Enter") {

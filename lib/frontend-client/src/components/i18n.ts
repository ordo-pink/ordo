...
export const init_i18n: TInitI18nFn = call_once(({ logger, commands, known_functions }) => {
	...
	commands.on("cmd.application.add_translations", ({ translations, lang }) => {
		...
		const prefixed_translations = keys_of(translations).reduce((acc, key) => ({ ...acc, [key]: translations[key] }), {})
	...
	translate.$ = translations$.pipe(combineLatestWith(current_language$)).pipe(map((_, index) => index))

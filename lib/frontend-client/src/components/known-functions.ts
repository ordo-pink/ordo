...
import { type TKnownFunctions, type TPermissions } from "@ordo-pink/core"
import { Option } from "@ordo-pink/option"

import { type TKnownFunction } from "../frontend-client.types"

// TODO: Extend query list :: data, etc.
export const init_known_functions = tau.call_once((f: TKnownFunction): TKnownFunctions => {
	...
	return {
		...
		has_permissions: (fid: symbol | null, permissions: Partial<TPermissions>) =>
			is_internal(fid) ||
			find_by_fid(fid)

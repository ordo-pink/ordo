...
export const init_background_task_display = (logger: TLogger, commands: Ordo.Command.Commands) => {
	...
	commands.on("cmd.application.background_task.reset_status", () => bg_task$.next(BackgroundTaskStatus.NONE))
	commands.on("cmd.application.background_task.start_loading", () => bg_task$.next(BackgroundTaskStatus.LOADING))
	commands.on("cmd.application.background_task.start_saving", () => bg_task$.next(BackgroundTaskStatus.SAVING))
...
const log_div_not_found = (logger: TLogger) => () => logger.error("#background-task-indicator div not found.")

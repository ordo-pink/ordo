...
const COMMAND_NAME = "gtd.mark-done"

type P = { commands: Client.Commands.Commands; data: DataProviders }
export const registerMarkDoneCmd = ({ commands, data }: P) => {
...
const registerCommand = ({ commands, data }: P) => {
	...
	commands.on<cmd.gtd.markDone>(COMMAND_NAME, handler)

	return () => {
		commands.off<cmd.gtd.markDone>(COMMAND_NAME, handler)
	}
}

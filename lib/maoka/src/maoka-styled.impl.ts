...
export const maoka_styled = HTML_TAGS.reduce(
	(acc, tag) => ({
		...acc,
		[tag]:
			<$Args extends Maoka.BaseArgs>(classes: string, f?: (args: Maoka.Args<$Args>) => void) =>
			(args: $Args) =>
	...
	{} as Record<
		...
		<$Args extends Maoka.BaseArgs>(classes: string, f?: (args: Maoka.Args<$Args>) => void) => (args: $Args) => Maoka.Component
	>,
)

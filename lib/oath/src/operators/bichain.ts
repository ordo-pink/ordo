...
export const bichain_oath =
	...
	(o: Oath<$TResolve, $TReject>): Oath<$TNewResolve, $TNewReject> =>
		new Oath<$TNewResolve, $TNewReject>(
			...
			(resolve, reject) =>
				o.fork(
					a => (o.is_cancelled ? reject(o.cancellation_reason as any) : on_reject(a).fork(reject, resolve as any)),
					b => (o.is_cancelled ? reject(o.cancellation_reason as any) : on_resolve(b).fork(reject, resolve)),
				),
			o._abort_controller,
		)

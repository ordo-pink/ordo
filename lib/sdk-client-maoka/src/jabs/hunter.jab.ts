...
export const track_on_create: <$Prey extends keyof Hunt.ToPreys<OrdoClient.Command.Preys>>(
	prey: $Prey,
...
export const track_on_mount: <$Prey extends keyof Hunt.ToPreys<OrdoClient.Command.Preys>>(
	prey: $Prey,
	gun: OrdoClient.Command.GunFor<$Prey>,
) => Maoka.Jab =
	(prey, gun) =>
	({ use }) => {
		...
		const handle_onmount = () => state.hunter.track(prey, gun)

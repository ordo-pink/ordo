...
export const new_ref: User.NewRef = (id, email) => email.split("@")[0].concat(id.split("-")[0])
export const new_user: User.NewUser = (email, ref, name = default_name(), sub = default_subscription(), ifs = [], p = null) =>
	result
		.of(uuid.create())
		.pipe(result.ops.chain(id => result.merge({ id, t: timestamp.create(), ref: ref ?? new_ref(id, email) })))

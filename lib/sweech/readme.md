...
- **x** - anything to match cases against. Returns an object that implements `Isweech`. In TypeScript, `Isweech` has two
  paramaters: the **TContext** which is inferred from the type of **x** and the **TResult** which holds all possible `Isweech`
  return value types. The **TResult** is transformed into a Union when you fold the **sweech** with a `.default`.

The `sweech.of_true` accepts no arguments and compares all case predicates against `true`.

The `sweech.of_false` accepts no arguments and compares all case predicates against `false`.

...
import { IAuthContextProps, reactKeycloakWebContext } from "$core/auth/context"
import type {
  AuthClient,
  AuthClientError,
  AuthClientEvent,
...
export function createAuthProvider<T extends AuthClient>(
  ...
  return class KeycloakProvider extends PureComponent<
    PropsWithChildren<AuthProviderProps<T>>,
    AuthProviderState
  > {
    state = {
      ...initialState,
    ...
    init() {
      ...
      authClient.init({ ...defaultInitOptions, ...initOptions }).catch(this.onError("onInitError"))
    }
    ...
    render() {
      ...
      return (
        <AuthContext.Provider value={{ initialized, authClient }}>{children}</AuthContext.Provider>
      )
    }
  }
}

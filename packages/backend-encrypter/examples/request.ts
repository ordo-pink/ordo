...
const makeRequest = (url: string) => (payload: string | Buffer): Promise<Buffer> => {
...
const tap = <T>(fn: (data: T) => void) => (data: T) => { fn(data); return data }
...
makeRequest("http://localhost:1337/encrypt")(payload)
  .then(tap((data) => { enryptedSlice = getSlice(data); enryptedLength = data.length }))

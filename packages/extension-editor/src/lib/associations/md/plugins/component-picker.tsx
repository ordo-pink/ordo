...
import { ActionListItem, useModal } from "@ordo-pink/react-utils"
import {
  $createParagraphNode,
  $getSelection,
  $isRangeSelection,
...
export default function ComponentPickerMenuPlugin(): JSX.Element {
  ...
  const checkForTriggerMatchSlash = useBasicTypeaheadTriggerMatch("!", {
    minLength: 0,
  })
    ...
    if (fullTableMatch) {
      ...
      options.push(
        new ComponentPickerOption(`${rows}x${columns} Table`, {
          ...
          keywords: ["table"],
          onSelect: () =>
            editor.dispatchCommand(INSERT_TABLE_COMMAND, { columns, rows } as unknown as {
              columns: string
              rows: string
            }),
        }),
          ...
          new ComponentPickerOption(`Heading ${n}`, {
            ...
            onSelect: () =>
              editor.update(() => {
                const selection = $getSelection()
                if ($isRangeSelection(selection)) {
                  $setBlocksType_experimental(selection, () => $createHeadingNode(`h${n}` as "h1"))
                }
              }),
          }),
      ),
  ...
  return (
    <>
      <LexicalTypeaheadMenuPlugin<ComponentPickerOption>
        ...
        triggerFn={checkForTriggerMatchSlash}
        options={options}
        menuRenderFn={(
          anchorElementRef,
          ...
          anchorElementRef.current && options.length
            ? ReactDOM.createPortal(
                <div className="absolute bg-neutral-200 dark:bg-neutral-900 p-2 rounded-lg shadow-lg w-64">

...
export default function EditorPage({ image, imageAlt = "", currentFile, children }: Props) {
  ...
  return Either.fromNullable(currentFile).fold(Null, (file) => (
    <div className="flex flex-col w-full relative">
      {image ? (
        <img
          className="absolute h-96 w-full top-0 left-0 right-0 object-cover"
          src={image}
          alt={imageAlt}
          title={imageAlt}
        />
      ) : (
        <div className="bg-gradient-to-tr from-red-200 via-pink-200 to-cyan-200 dark:from-zinc-600 dark:via-stone-600 dark:to-slate-600 w-full top-0 left-0 right-0 h-96 absolute" />
      )}
      ...
      <div className="mt-96 py-4 self-center prose prose-pink prose-headings:text-neutral-700 dark:prose-headings:text-neutral-300 w-full h-full max-h-screen caret-purple-800 dark:prose-invert dark:caret-purple-200 cursor-text">

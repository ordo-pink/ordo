	...
	(process.platform === "darwin" ? MAC_MENU : []).concat([
		...
		},
		{
			...
			submenu: [
				...
				separator,
				toMenuItem(getCommandByName("Editor", state)),
				toMenuItem(getCommandByName("Graph", state)),
				toMenuItem(getCommandByName("Find In Files", state)),
				separator,
				toMenuItem(getCommandByName("Settings", state)),
			],
		},
	] as any)

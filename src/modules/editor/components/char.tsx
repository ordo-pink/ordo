      ...
      (e: React.MouseEvent) => {
        ...
        if (e.altKey) {
          const found = current.tab.caretPositions.findIndex(
            (position) =>
              position.start.character === char.position.character && position.start.line === char.position.line,
          );

          if (found !== -1) {
            const payload = current.tab.caretPositions
              .slice(0, found)
              .concat(current.tab.caretPositions.slice(found + 1));

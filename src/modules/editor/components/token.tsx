...
export const Token = React.memo(
  ({ token, lineIndex }: { token: Node; lineIndex?: number }) => {
    ...
    const alwaysShowMarkdownSymbols = useAppSelector(
      (state) => state.app.userSettings.editor.alwaysShowMarkdownSymbols,
    );
        ...
        {isNodeWithChildren(token) && (
          <span>
            {(alwaysShowMarkdownSymbols || isCurrentLine) &&
              token.openingChars.map((char) => (
                <Character key={`${token.id}-${char.position.character}`} char={char} />
              ))}
            {token.children.map((child) => (
              <Token key={`${child.id}`} token={child} lineIndex={lineIndex} />
            ))}
            {(alwaysShowMarkdownSymbols || isCurrentLine) &&
              token.closingChars.map((char) => (
                <Character key={`${token.id}-${char.position.character}`} char={char} />
              ))}
          </span>
        )}

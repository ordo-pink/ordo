...
import { NoOp, noOpFn, FoldVoid } from "@utils/functions";
  ...
  ({ node }) => {
    ...
    React.useEffect(() => {
      ...
      Either.fromNullable(tree)
        ...
        .map((tree) =>
          setTasks(
            collectFiles(tree).reduce(
              (acc, task) => (task.type === "document" ? { ...acc, [task.path]: task } : acc),
              {},
            ),
          ),
        )
    ...
    const handleColumnInputChange = (e: React.ChangeEvent<HTMLInputElement>) =>
      setCreateInputColumnValue(e.target.value);

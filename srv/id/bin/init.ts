...
const generateKeyPairP = async () => {
	const { privateKey, publicKey } = await crypto.subtle.generateKey({ name: "ECDSA", namedCurve: "P-384" }, true, [
		"sign",
		"verify",
	])
...
const main = () => {
	...
	void read_file0("./.env", "utf-8")
		...
		.pipe(ops0.map(lines => lines.reduce((acc, line) => ({ ...acc, [line[0]]: line[1] }), {} as Record<string, string>)))
		.pipe(
			ops0.chain(env =>
		...
		.pipe(ops0.map(env => keys_of(env).reduce((acc, key) => acc.concat(`${key}=${env[key]}\n`), "")))

...
export const init_modal = ({ commands, logger }: P) => {
	...
	modal$.pipe(pairwise()).subscribe(([prev_state_option, state_option]) => {
		Switch.Match(state_option)
			...
			.default(() => {
				...
				modal_overlay_element.onclick = () => unmount(state.on_unmount)
				...
				if (state.show_close_button) {
					...
					close_button.onclick = () => unmount(state.on_unmount)
	...
	logger.debug("Modal initialised.")
}
...
const on_modal_show: Client.Commands.Handler<Client.Modal.HandleShowPayload> = payload => {
	const show_close_button = payload.options?.show_close_button ?? true
	const on_unmount = payload.options?.on_unmount ?? (() => void 0)

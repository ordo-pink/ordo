...
export default function RemoveFileModal({ file }: Props) {
	const { commands } = useSharedContext()

	const handleRemoveFile = () => {
		commands.emit<cmd.data.file.remove>("data.remove-file", file.path)
		commands.emit<cmd.modal.hide>("modal.hide")
	}

	const tTitle = "Remove file"
	const tText = `Are you sure you want to remove "${file.path}"? This action is irreversible.`
